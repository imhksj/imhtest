 ...
 * =============================================================================
 * 프로젝트명 : woowoneng
 * 화   일  명 : ParamUtil.java
 * 기        능 : request 값 map으로 파싱
 * -----------------------------------------------------------------------------
 *                              변경 사항				                     
 * -----------------------------------------------------------------------------
 *    변경일자       	                             변경자(작성자)                 	  변경 내역                 
...
public class ParamUtil<K, V> extends HashMap<K, V> implements Serializable {
	
	static final long serialVersionUID = 8750411155467048253L;
	
	static final Log logger = LogFactory.getLog(ParamUtil.class.getName());
	...
	public boolean isMultipartForm(ServletRequest request) {
	...
	private void initMap(ServletRequest request) {
		...
		if (isMultipartForm(request)) {
			...
			while (itr.hasNext()) {
				...
				put((K)filename, (V)multiRequest.getAttribute(filename));
				
			}
			
		}
		
		java.util.Enumeration<?> att_names = request.getAttributeNames();
		while (att_names.hasMoreElements()) {
			String name = (String)att_names.nextElement();
			String values = (request.getAttribute(name)).toString();
			put((K)name, (V)values);
		}
	...
	public int[] getIntArray(String name, int defaultVal) {
		...
		if(tmpObj instanceof int[]) {
			...
			for(int k=0; k<objs.length; k++) {
				try {
					retVals[k] = Integer.parseInt( StringUtil.nullToBlank(String.valueOf(objs[k]), "0") );
				} catch(Exception e) {
