...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		setChecked("prodChoicSpec", prodChoicSpec, 0);

		if(totalCount <= 0 || totalPage < 2)	$("#loadListBtn").hide();
		
		if(${empty sessionScope.member || sessionScope.member eq ''?0:1} == 1){
	...
	function fnLoadList(){
		...
		if(totalPage >= reqPage){
			...
			$.get( "productListAjx.do", {reqPage:reqPage, searchKeyword:searchKeyword, prodChoicSpec:prodChoicSpec}, function(htmlData){
...
<div class="content">
	<div class="top_area">
		<h2 class="t_tit">${param.searchKeyword} 제품검색</h2>

		<div class="fl_right">
			<span class="rdo"><input type="radio" id="rdo_01" value="" checked="checked" title="" name="prodChoicSpec" onChange="location.href='?searchKeyword=${param.searchKeyword}'"> <label for="rdo_01">전체제품</label></span>
			<span class="rdo"><input type="radio" id="rdo_02" value="클린사업장 지원제품" title="" name="prodChoicSpec" onChange="location.href='?searchKeyword=${param.searchKeyword}&prodChoicSpec=클린사업장 지원제품'"> <label for="rdo_02">클린사업장 지원제품</label></span>
			<span class="rdo"><input type="radio" id="rdo_03" value="클린사업장 지원제품" title="" name="prodChoicSpec" onChange="location.href='?searchKeyword=${param.searchKeyword}&prodChoicSpec=정부조달사업 등록제품'"> <label for="rdo_03">정부조달사업 등록제품</label></span>
		</div>
	</div>
					...
					<li>
							<c:choose>
								<c:when test="${empty sessionScope.member || sessionScope.member eq ''}">
									<a href="javascript:alert('로그인 후 주문가능합니다.');">	
								</c:when>
								<c:otherwise>
