...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//수정이면
		if("${!empty press}" != "false")	$("#f").attr("action","modiPress.do");
	...
	function addPress(){
		...
		if(!nullChkAlert($("#title").val() ,"기사제목은 필수 입력 항목입니다."))	return;
		if(!nullChkAlert($("#pressDate").val() ,"보도일은 필수 입력 항목입니다."))	return;
		if(!nullChkAlert($("#pressName").val() ,"언론사명은 필수 입력 항목입니다."))	return;
	...
	function delPress(){
		...
		$.get("delPress.do?idx=${press.idx}", function(jsonData){
	...
	function fnGoList(){
		...
		location.href = "press_list.do?reqPage=${param.reqPage}";
		...
		<div class="content">
			<div class="h_area">
				...
				<div class="al_right">
					...
					<button type="button" class="btn_base" onClick="addPress();">${!empty press?'수정':'저장'}</button>
				</div>
			</div>
			
			<div class="table_wrap detail">
				<form action="addPress.do" method="post" name="f" id="f">
					<input type="hidden" name="fileDelIdx" id="fileDelIdx" value="">
					<input type="hidden" name="idx" id="idx" value="${!empty press?press.idx:'0'}">
					<!-- table -->
					<table border="1">
						...
						<thead>
							<tr>
								<th scope="col" colspan="4">언론보도 ${!empty press?'수정하기':'등록하기'}</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<th scope="row">ㆍ기사제목<span class="imp">*</span></th>
								<td colspan="3"><input type="text" value="${press.title}" id="title" title="" name="title" style="width:575px" /></td>
							</tr>
							<tr>
								<th scope="row">ㆍ보도일<span class="imp">*</span></th>
								<td colspan="3"><input type="text" value="${press.pressDate}" id="pressDate" title="" name="pressDate" style="width:575px" /></td>
							</tr>
							<tr>
								<th scope="row">ㆍ언론사명<span class="imp">*</span></th>
								<td colspan="3"><input type="text" value="${press.pressName}" id="pressName" title="" name="pressName" style="width:575px" /></td>
							</tr>
							<tr>
								<th scope="row">ㆍ기사링크</th>
								<td colspan="3"><input type="text" value="${press.pressLink}" id="pressLink" title="" name="pressLink" style="width:575px" /></td>
							</tr>
							<tr>
								<th scope="row">ㆍ내용</th>
								<td colspan="3"><textarea placeholder="내용입력" id="content" name="content" rows="" cols="" style="width:583px; height:190px">${press.content}</textarea></td>
							</tr>
							<tr>
								...
								<td>
									<span class="b_file">
										...
										<input type="hidden" id="fileGroup" name="fileGroup" value="PR/PRESS">
			...
			<div class="btn_wrap cen line">
				<c:if test="${!empty press}">
					<button class="btn_base red" onClick="delPress();">삭제</button>				
				</c:if>
